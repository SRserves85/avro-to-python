...
""" avro python class for file: Point """
...
class Point(object):
    ...
    def __init__(self, obj: Union[str, dict, 'Point'] = None) -> None:
        if obj is None:
            return
        ...
        if isinstance(obj, str):
            ...
            raise TypeError(
                f"{type(obj)} is not in ('str', 'dict', 'Point')"
            )
        ...
        if isinstance(values, list):
            for element in values:
                if isinstance(element, (float, int)):
                    ...
                    raise TypeError(
                        f"Type for 'coordinates' should be '(float, int)' but was: {element}"
                    )
        else:
